...
const HELP_SHA256 = '8fb1cfa0b64c820b75c7738ff0c218e020c1ae0d9a495db07cc2f7384d880307';

const sha256 = (value) => createHash('sha256').update(value).digest('hex');
const lineCount = (file) => readFileSync(file, 'utf8').trimEnd().split('\n').length;

function run(args) {
  return spawnSync(process.execPath, [WEBMCP_BROWSER_BIN, ...args], {
    cwd: ROOT,
    encoding: 'utf8',
...
test('public Browser Kit entrypoints are compatibility shims no longer than 15 lines', () => {
  assert.equal(existsSync(WEBMCP_BIN), false, 'legacy webmcp bin must not exist');
...
test('browser package exposes only the webmcp-browser bin', () => {
  const manifest = JSON.parse(readFileSync(path.join(ROOT, 'package.json'), 'utf8'));
  assert.deepEqual(manifest.bin, { 'webmcp-browser': 'bin/webmcp-browser.mjs' });
});
