...
// ── A3 slice v1: read-through shim to the in-process Governor (ADR 0012) ─
// Read-only projection of Governor resource state; never locks, never
// writes, never mints leases. Legacy pool behavior is intact: the shim only
// surfaces inside `doctor` when WEBMCP_GOVERNOR_READTHROUGH=1, and is always
// available as the exported readGovernorReadThrough helper.
export function getGovernorReadThroughStatePath() {
...
export async function readGovernorReadThrough({ statePath = null } = {}) {
  const resolved = statePath || getGovernorReadThroughStatePath();
  const { GovernorRepository } = await import('../../profile-governor/repository.mjs');
  let repository = null;
  try {
    repository = new GovernorRepository({ statePath: resolved });
  } catch (error) {
    return { present: existsSync(resolved), ok: false, error: error?.code || 'UNAVAILABLE' };
  }
  try {
    const state = repository.read();
    const resources = Object.values(state?.resources || {}).map((resource) => ({
      profileAlias: resource?.profileAlias ?? null,
      state: resource?.state ?? null,
      stateReasonCode: resource?.stateReasonCode ?? null,
      fenceEpoch: resource?.fenceEpoch ?? null,
      livenessSummary: resource?.livenessSummary ?? null,
      needsReconciliation: Boolean(resource?.needsReconciliation),
    }));
    return { present: true, ok: true, resourceCount: resources.length, resources };
  } catch (error) {
    return { present: existsSync(resolved), ok: false, error: error?.code || 'UNREADABLE' };
  } finally {
    try { repository.close?.(); } catch { /* read-only shim never fails close */ }
  }
}

function isGovernorReadThroughEnabled() {
  return process.env.WEBMCP_GOVERNOR_READTHROUGH === '1';
}
...
export async function runProfilePool(args) {
  ...
  try {
    ...
    if (subcommand === 'acquire') { command = 'profile-pool.acquire'; data = await cmdAcquire(rest); }
      ...
      command = 'profile-pool.doctor';
      data = cmdDoctor(rest);
      if (isGovernorReadThroughEnabled()) data.governor = await readGovernorReadThrough();
    }
