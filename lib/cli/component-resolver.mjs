...
export function getRunnerBin() {
  ...
  // Source-layout fallback is dev-only. The sibling source checkout resolves
  // ONLY with the explicit WEBMCP_DEV_SOURCE_FALLBACK=1 opt-in (which wins
  // even under NODE_ENV=production); every other combination — production or
  // unset NODE_ENV — resolves the installed package or reports unavailable.
...
function allowSourceFallback() {
  if (process.env.WEBMCP_DEV_SOURCE_FALLBACK === '1') return true;
  return false;
}
