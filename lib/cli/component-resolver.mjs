...
export function getRunnerBin() {
  ...
  // Source-layout fallback is dev-only. Explicit opt-in via
  // WEBMCP_DEV_SOURCE_FALLBACK=1 always wins; otherwise a production runtime
  // (NODE_ENV=production, as in a packaged app) must never resolve a sibling
  // source checkout. Dev and test processes keep the sibling resolution.
