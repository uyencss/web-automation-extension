import process from 'node:process';
import { spawn } from 'node:child_process';
import { existsSync } from 'node:fs';
import { resolveCliBin } from './component-resolver.mjs';
import { printMcpHelp } from './help.mjs';

async function runSkills(args) {
  const cliBin = resolveCliBin();
  if (!cliBin || !existsSync(cliBin)) {
    console.error([
      'WebMCP CLI not found.',
      'Install @gyga-browser/webmcp-cli, run from the webmcp-automation-kit checkout, or set WEBMCP_CLI_BIN.',
    ].join('\n'));
    return 1;
  }

  const child = spawn(process.execPath, [cliBin, 'skills', ...args], {
    cwd: process.cwd(),
    env: { ...process.env },
    stdio: 'inherit',
  });

  return new Promise((resolveExitCode) => {
    child.on('error', (err) => {
      console.error(`Failed to start WebMCP CLI: ${err.message}`);
      resolveExitCode(1);
    });
    child.on('exit', (code, signal) => {
      if (signal) {
        console.error(`WebMCP CLI exited after signal ${signal}`);
        resolveExitCode(1);
        return;
      }
      resolveExitCode(code ?? 1);
    });
  });
}

const exit = (exitCode) => ({ handled: true, exitCode });
const done = { handled: true };

const COMMANDS = new Map([
  ['mcp', async (args) => {
    if (args.includes('--help') || args.includes('-h') || args[0] === 'help') {
      printMcpHelp();
      return done;
    }
    await import('../../server/mcp_server.mjs');
    return done;
  }],
  ['doctor', async (args) => exit(await (await import('./commands/doctor.mjs')).runDoctor(args))],
  ['bootstrap', async (args) => exit(await (await import('./commands/bootstrap/index.mjs')).runBootstrap(args))],
  ['gateway', async (args) => {
    await (await import('./gateway-client.mjs')).runGateway(args);
    return done;
  }],
  ['profiles', async (args) => {
    await (await import('./commands/chrome.mjs')).runProfiles(args);
    return done;
  }],
  ['profile-pool', async (args) => exit(await (await import('../profile-pool/cli.mjs')).runProfilePool(args))],
  ['launch', async (args) => {
    await (await import('./commands/chrome.mjs')).runLaunch(args);
    return done;
  }],
  ['close', async (args) => {
    await (await import('./commands/chrome.mjs')).runClose(args);
    return done;
  }],
  ['quit', async (args) => {
    await (await import('./commands/chrome.mjs')).runQuit(args);
    return done;
  }],
  ['workflow', async (args) => exit(await (await import('./commands/delegates.mjs')).runWorkflow(args))],
  ['ai', async (args) => exit(await (await import('./commands/delegates.mjs')).runAi(args))],
  ['site', async (args) => exit(await (await import('./commands/delegates.mjs')).runSite(args))],
  ['automation', async (args) => exit(await (await import('./commands/delegates.mjs')).runAutomation(args))],
  ['project', async (args) => exit(await (await import('./commands/project/index.mjs')).runProject(args))],
  ['mobile', async (args) => exit(await (await import('./commands/delegates.mjs')).runMobile(args))],
  ['adb', async (args) => exit(await (await import('./commands/delegates.mjs')).runMobile(args))],
  ['captcha', async (args) => exit(await (await import('./commands/delegates.mjs')).runCaptcha(args))],
  ['skills', async (args) => exit(await runSkills(args))],
  ['store', async (args) => exit(await (await import('./commands/delegates.mjs')).runSite(args, { legacyAlias: true }))],
  ['health', async (args) => {
    await (await import('./gateway-client.mjs')).printHealth({ json: args.includes('--json') });
    return done;
  }],
  ['call', async (args) => {
    const [method, rawParams] = args;
    if (!method) {
      console.error('Usage: webmcp call <method> [jsonParams]');
      return exit(1);
    }
    await (await import('./gateway-client.mjs')).callGateway(method, rawParams);
    return done;
  }],
  ['vault', async (args) => exit(await (await import('./commands/delegates.mjs')).runVault(args))],
  ['extension-info', async (args) => {
    (await import('./commands/extension.mjs')).runExtensionInfo(args);
    return done;
  }],
  ['extension-path', async () => {
    (await import('./commands/extension.mjs')).runExtensionPath();
    return done;
  }],
]);

export async function routeCommand(command, args) {
  const handler = COMMANDS.get(command);
  if (!handler) return { handled: false };
  return handler(args);
}
