...
import { validateEventIntegrity, validateRedactedEvent } from '../../../profile-governor/events.mjs';
...
const LEASE_ID_SYNTAX = /^lease_[a-z0-9]{8,}$/;
...
function validateRecoveryPlanBinding(resource, lease, events = [], receipts = []) {
  ...
  if (resource.currentLeaseId === lease.leaseId) {
    ...
    for (const event of currentEpochQuarantineEvents) {
      if (typeof event.stateReasonCode === 'string' && event.stateReasonCode) {
        candidateReasons.add(event.stateReasonCode);
      }
    }
    ...
    const hasEarlierQuarantineEvent = Array.isArray(events) && events.some(
      (e) =>
        e &&
        e.leaseId === lease.leaseId &&
        e.profileAlias === lease.profileAlias &&
    ...
    const hasCurrentEpochQuarantineEvent = currentEpochQuarantineEvents.length > 0;

    if (hasEarlierQuarantineEvent && hasCurrentEpochQuarantineEvent) {
      return;
    }
  ...
  if (resource.currentLeaseId === null) {
    ...
    for (const event of priorQuarantineEvents) {
      if (
        ...
        computeRecoveryPlanDigest({
          ...
          reasonCode: event.stateReasonCode,
        })
      ) {
        matches = true;
        break;
    ...
    // paired/equal historical digest as unverified read-only history.
    return;
  }
...
function validateDurableState(value) {
  ...
  for (const resource of Object.values(value.resources)) {
    if (resource.currentLeaseId) {
      ...
      if (!lease || lease.physicalResourceId !== resource.physicalResourceId) {
...
export async function runGovernor(args) {
  ...
  // Writer paths: strict --lease-id handling + registry-first fail-closed.
  if (sub === 'release') {
    ...
    if (!LEASE_ID_SYNTAX.test(rawLeaseId)) {
