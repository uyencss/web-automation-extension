...
function renderServiceTemplate(id, role) {
  ...
  if (process.platform === 'darwin') {
    return [
      ...
      `    <string>${resolve(ROOT, 'bin', 'webmcp-browser.mjs')}</string>`,
  ...
  if (process.platform === 'win32') {
    return [
      ...
      `  <arguments>${resolve(ROOT, 'bin', 'webmcp-browser.mjs')} gateway start</arguments>`,
  ...
  return [
    ...
    `ExecStart=${process.execPath} ${resolve(ROOT, 'bin', 'webmcp-browser.mjs')} gateway start`,
    'Restart=on-failure',
    'UMask=0077',
    '',
    '[Install]',
