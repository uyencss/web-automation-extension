...
export async function runProjectSchedule(args) {
  ...
  if (subcommand === 'plan') {
    ...
    if (json) {
      ...
      return plans.some((p) => p.ok === false) ? 1 : 0;
    }
    ...
    for (const p of plans) {
      if (p.ok === false) {
        hasFailure = true;
        console.error(`\x1b[31m✗\x1b[0m Failed to plan \x1b[1m${p.id}\x1b[0m: ${p.code || 'PLAN_FAILED'} - ${p.message || p.reason || 'planning failed'}`);
        continue;
      }
